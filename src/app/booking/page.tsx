        ...
        {STEPS.map((label, i) => (
          <div key={i} className="flex flex-col items-center flex-1">
            <div className="flex items-center w-full">
              <div className={`h-px flex-1 ${i === 0 ? "invisible" : i <= step ? "bg-primary" : "bg-border"}`} />
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium shrink-0 ${i <= step ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                {i + 1}
              </div>
              <div className={`h-px flex-1 ${i === STEPS.length - 1 ? "invisible" : i < step ? "bg-primary" : "bg-border"}`} />
            </div>
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-3">
            {services.map((s) => (
              <Button
                key={s.id}
                variant={selectedService?.id === s.id ? "default" : "outline"}
                // variant="outline"
          ...
          <Button className="w-full" size="lg" disabled={!selectedService} onClick={() => setStep(1)}>
            下一步
          </Button>
        </div>
      )}
